...
4. Dark-mode neumorphic palette is entirely to-be-designed. — *resolved: both modes shipped.*
5. Whether Fraunces/serif has any role is unconfirmed. — still open; `--font-display` is still Fraunces and still used by `h1`/`h2`.
6. The semantic hues in §3.2 are `[INFERRED]` from a compressed board. The shipped ramps preserve those **hues** but re-solve **lightness** for WCAG AA, so the shipped fills are darker than the reference hexes (most visibly `--warning`: amber at the reference's L≈71% cannot reach 3:1 on cream paper). Re-sample against the full-res reference before treating any of them as brand values.
...
- ✅ **Inventory completed (done):** the five §10 components that were specified but never built now exist — `textarea.tsx` (inset well, mirrors `Input`'s `error` API), `progress.tsx` (inset groove + accent fill; determinate and indeterminate), `avatar.tsx` (raised disc + shared `getInitials`), `alert.tsx` (in-flow raised banner with a 4px semantic edge — *not* a floating toast shadow, per §4), `breadcrumbs.tsx` (ink-only, flush with the paper).
- ✅ **Wired into real screens (done):** `ImportWizard` now uses the shared `Stepper` (it had a flat hand-rolled one) and `Alert` for its skipped-row report; `TopBar` uses `Breadcrumbs` + `Avatar`; `Sidebar` uses `Avatar` (its duplicate `getInitials` deleted); `HoldingsTable`'s weight bar uses `Progress`; all four hand-rolled `<textarea>` blocks use `Textarea`. Two of those four (AI copilot, transactions) were still on glass-era `focus:ring-*`, which the global neumorphic `box-shadow` silently overrode — they had **no visible focus ring** until this pass moved them onto the shared outline-based one.
- ✅ **Semantic status repalette (done):** §3.2 specified earthy semantics (olive 136, amber 81, warm brick 34, muted slate-teal 230), but `tokens.css` still pointed `--success/--warning/--danger/--info` at the glass-era cool `blue-100`/`green-100`/`red-100` primitives. Invisible at badge scale; obvious once `Alert` rendered them full-width. Primitives are now `--sage-*/--amber-*/--brick-*/--slate-*`, and the pale `-100` tints pull ~25% toward the paper hue so a banner reads as *stained paper*. `--slate-100` deliberately carries less chroma than its siblings — it is the only cool hue, and simultaneous contrast against warm paper makes equal chroma read far louder.
- ✅ **Financial contrast fix (done):** the earlier contrast pass measured body/muted/accent text but not `--positive`/`--negative`, which are body text in dense tables. Measured, they were **below AA on the surfaces they actually render on**: light-mode `--positive` 4.07 on paper and 3.53 on table headers, `--negative` 3.97 on table headers, dark-mode `--negative` 4.05 on cards. Both are now solved against the worst-case ground in each theme (light: `--surface-muted`; dark: `--surface-elevated`) so AA holds everywhere lighter. Gains adopt the reference's olive hue (136); losses stay redder (28) to hold them apart from the terracotta accent (46). Measured ratios recorded in `tokens.css`.

**The tactile system is now complete across the component library**, and every component named in the §10 inventory exists and is used by at least one real screen. Remaining is optional: the still-open terracotta-primary-button toggle.
